...
def get_flows(base_url):
    ...
    '''
    result = []
    flows = get_request(base_url, 'flows')
    if flows == -1:
        return
    if len(flows) == 0:
...
def get_flowTblSize(base_url, tableId):
    '''
    ...
    flows = get_request(base_url, request)
    if flows  == -1:
        return
    for flow in flows['flows']:
        size = size +1 
...
def get_flowsByAppId(base_url, appId):
    '''
    ...
    '''
    result = []
    appIds = get_flowsAppIds(base_url)
    if len(appIds) <= 0:
        print('flow rules not found!\n')
        return result
    if appId not in appIds:
        ...
        return result
    return get_request(base_url, "flows/application/" + appId)

def get_flowsByDeviceId(base_url, dId):
    '''
...
def get_flowByFlowId(base_url, dId, fId):
    '''
...
def clear_flowsByAppId(base_url, appId):
    '''
...
def post_flows(base_url, ifile):
    ...
    with codecs.open(ifile, "r", encoding="utf-8") as f:
        newFlow = json.load(f)
    url = base_url + "flows"
    response = post_request(url, data=json.dumps(newFlow))
    print(response)
    f.close()
